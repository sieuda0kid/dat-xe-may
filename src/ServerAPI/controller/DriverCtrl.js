...
var send_request=function(soket_driver,request,check)
{
    return new Promise((resolve, reject) => {
        ...
        soket_driver.on("destroy_request",function(data){
            soket_driver.premission=false;
           console.log(`----------------> Driver ${soket_driver.user.username} đã hủy chuyến đi`);
           clearTimeout(action);
           thoi_han=true
           reject(thoi_han);
        });
     		...
     		(function(ind) {
     			var action=setTimeout(function(){
     				...
     				if(tt==0)
     				{
     					...
     					.then(result=>{
                            console.log(`--------> Driver ${arrDistance[t].user.username} đã nhận chuyến đi !!!!!!!`);
     					...
     					.catch(err=>{
     						console.log(err);
     						console.log(`driver ${arrDistance[t].user.id}  không nhận`);
     						t++;
     					});
     					}
     				}
